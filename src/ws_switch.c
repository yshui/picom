...
struct ws_switch {
	...
	/// and the layout manager ring isn't advanced during them, so after the switch
	/// the buffer age based damage computation could reference pre-switch layouts.
	/// Forcing full repaints for a few frames repopulates the ring with post-switch
	/// layouts before the damage tracking is trusted again.
...
static void
ws_switch_layout_position(unsigned index, unsigned orientation, unsigned columns,
                          unsigned rows, unsigned starting_corner, int *col, int *row) {
	if (orientation == 0) {
...
ws_switch_direction_for(session_t *ps, long old_desktop, long new_desktop) {
	...
	if (ws->has_layout) {
		...
		ws_switch_layout_position((unsigned)old_desktop, ws->layout_orientation,
		                          ws->layout_columns, ws->layout_rows,
		                          ws->layout_corner, &old_col, &old_row);
		ws_switch_layout_position((unsigned)new_desktop, ws->layout_orientation,
		                          ws->layout_columns, ws->layout_rows,
		                          ws->layout_corner, &new_col, &new_row);
		if (new_col > old_col) {
	...
	return new_desktop > old_desktop ? WS_SWITCH_DIRECTION_RIGHT : WS_SWITCH_DIRECTION_LEFT;
}
...
bool ws_switch_render(session_t *ps, uint64_t render_start_us, bool window_animation) {
	...
	case WS_SWITCH_CAPTURE_PRE: {
		...
		bool ok =
		    ws_switch_render_normal(ps, render_start_us, NULL, true, NULL, NULL);
		if (!ok) {
	...
	case WS_SWITCH_WAITING: {
		...
		if (settled || timed_out) {
			if (timed_out && !settled) {
				log_warn("The new desktop didn't settle in time, "
				         "starting "
	...
	case WS_SWITCH_ANIMATING: {
		double progress = (double)(ws_switch_now_ms() - ws->anim_start_ms) /
		                  (double)max2(ps->o.workspace_animation_duration, 1);
		progress = min2(progress, 1.0);
		if (progress >= 1.0) {
			bool frame_changed = false;
			bool succeeded = ws_switch_render_normal(
			    ps, render_start_us, NULL, true, NULL, &frame_changed);
			ws->full_repaint_frames =
