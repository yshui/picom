...
paint_region(session_t *ps, const struct managed_win *w, int x, int y, int wid, int hei,
	...
	render(ps, x, y, dx, dy, wid, hei, fullwid, fullhei, opacity, argb, neg,
			(w && !win_is_fullscreen(ps, w) && !c2_match(ps, w, ps->o.rounded_corners_blacklist, NULL) ? w->corner_radius : 0),
...
bool init_render(session_t *ps) {
	...
	// Blur filter
	if (ps->o.blur_method && ps->o.blur_method != BLUR_METHOD_KERNEL && ps->o.blur_method != BLUR_METHOD_KAWASE) {
	...
	if (ps->o.blur_method == BLUR_METHOD_KERNEL || ps->o.blur_method == BLUR_METHOD_KAWASE) {
		ps->blur_kerns_cache =
