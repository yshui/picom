...
bool x_extensions_init(struct x_connection *c) {
	...
	// The NameWindowPixmap request was introduced in the X Composite extension
	// v0.2.
	...
	if (!composite || (composite->major_version == 0 && composite->minor_version < 2)) {
		log_fatal("The X server doesn't support the X Composite extension "
		          "v0.2.");
	...
	// According to the X Damage extension specification:
	...
	// According to the X Fixes extension specification:
	// "The client must negotiate the version of the extension before executing
	...
	if (extension && extension->present) {
		...
		auto sync = xcb_sync_initialize_reply(
		    ...
		    NULL);
		if (sync && ((sync->major_version == 3 && sync->minor_version >= 1) ||
		             sync->major_version > 3)) {
			c->e.has_sync = true;
